...
//      this.refresh()
    ...
    methods: {
      ...
      delSelect() {
        let r = this.beforeDel(this.select_node);
        if (!r && r != undefined) {
            this.$notify({
                type: 'error',
                title: '操作提示',
      ...
      editMsg(data, node, e) {
        ...
        if (this.edit_name.replace(/^\s+|\s+$/g, '')) {
          if (!data.id) {
            ...
            let r = this.beforeAdd(virtualNode, params);
            if (!r && r != undefined) {
                this.$notify({
                    type: 'error',
                    title: '操作提示',
          ...
          let r = this.beforeUpdate(node, params);
          if (!r && r != undefined) {
              this.$notify({
                  type: 'error',
                  title: '操作提示',
      ...
      handleNodeClick(nodeData, node, own) {
        this.$emit('node-click', nodeData, node, own);
      }
    }
  }
</script>
